 ...
 */
export class BarrloItem extends Item {
    ...
    async prepareDerivedData() {
        ...
        itemData.enrichedDescription = await TextEditor.enrichHTML(itemData.description, {async: true});
    }
    ...
    getTags() {
        ...
        switch (this.system.type) {
            case 'weapon':
                ...
                if (data.missile) {
                    wTags += formatTag(data.range.short + '/' + data.range.long, 'fa-bullseye');
                }
                return wTags;
            case 'armor':
    ...
    async _assetAttack(isOffense) {
        ...
        const diceData = Roll.fromTerms([foundry.dice.terms.PoolTerm.fromRolls([attackRolls[0], attackRolls[1]])]);
        const attackKey = isOffense ? 'WWN.faction.attack-roll' : 'WWN.faction.counter-roll';

        const assetsWithLocationNotes = this.actor.items.filter(
            i =>
                i.id != this.id &&
                i.type == 'asset' &&
                i.system.location === this.system.location &&
                i.system.locationRoll
        );
    ...
    async _assetSearch(targetType) {
        ...
        const otherActiveFactions = game.actors?.filter(
            i => i.type === 'faction' && i.system.active == true && this.actor?.id != i.id
        );
        ...
        const _rollAssetForm = async html => {
            ...
            const attackedAssetId = form.querySelector(assetFormName)?.value;
            ...
            const attackedAssetsWithLocationNotes = attackedFaction.items.filter(
                i => i.type == 'asset' && i.system.location === this.system.location && i.system.locationRoll
            );
            const attackingAssetsWithLocationNotes = this.actor.items.filter(
                i =>
                    i.id != this.id &&
                    i.type == 'asset' &&
                    i.system.location === this.system.location &&
            ...
            const defenseRolls = await attackedAsset.getAssetAttackRolls(false);
            ...
            if (!hitRoll || hitRoll == undefined || !hitRoll.total || !defRoll.total) {
                return;
            }
