...
export class BarrloActor extends Actor {
    /**
    ...
    prepareData() {
        ...
        if (this.type === 'faction') {
            ...
            const assets = this.items.filter(i => i.type === 'asset');
            const cunningAssets = assets.filter(i => i.system['assetType'] === 'cunning');
            const forceAssets = assets.filter(i => i.system['assetType'] === 'force');
            const wealthAssets = assets.filter(i => i.system['assetType'] === 'wealth');
    ...
    async createEmbeddedDocuments(embeddedName, data = [], context = {}) {
        ...
        data.map(item => {
            if (item.img === undefined) {
                item.img = BarrloItem.defaultIcons[item.type];
            }
        });
    ...
    rollSave(save, options = {}) {
        ...
        const rollMethod = this.type === 'character' ? BarrloDice.RollSave : BarrloDice.Roll;
    ...
    computeEncumbrance() {
        ...
        if (game.settings.get('wwn', 'currencyTypes') === 'currencybx') {
            const coinWeight =
                (data.currency.cp + data.currency.sp + data.currency.ep + data.currency.gp + data.currency.pp) / 100;
    ...
    computeResources() {
        ...
        const torchArray = this.items.filter(i => i.name.toLowerCase() === 'torch');
        const rationsArray = this.items.filter(i => i.name.toLowerCase().includes('rations'));
    ...
    enableSpellcasting() {
        ...
        arts.length > 0 ? (this.system.spells.artsEnabled = true) : (this.system.spells.artsEnabled = false);
        spells.length > 0 ? (this.system.spells.spellsEnabled = true) : (this.system.spells.spellsEnabled = false);
    }
    ...
    async computeSlots() {
        ...
        Object.keys(slots).forEach(level => (this.system.spells.slots[level].used = 0));
    ...
    computeTreasure() {
        ...
        // Compute treasure
        let total = 0;
        const treasures = this.items.filter(i => i.type === 'item' && i.system.treasure && !i.system.personal);
    ...
    computePersonalTreasure() {
        ...
        // Compute treasure
        let total = 0;
        const treasures = this.items.filter(i => i.type === 'item' && i.system.personal);
        treasures.forEach(item => {
            total += item.system.quantity * item.system.price;
        });
    ...
    computeModifiers() {
        ...
        Object.keys(scores).map(score => {
            this.system.scores[score].mod =
                this.system.scores[score].tweak + BarrloActor._valueFromTable(table, scores[score].value);
        });
    }
