export class BarrloAdjustCurrency extends FormApplication {
    static get defaultOptions() {
        const options = super.defaultOptions;
        options.id = 'sheet-tweaks';
        options.template = 'systems/wwn/templates/actors/dialogs/adjust-currency.html';
        options.width = 280;
        return options;
    }
    ...
    async _adjustCurrency(ev) {
        ...
        updatedCurrency.cp =
            parseInt($(ev.currentTarget.parentElement.parentElement).find('input[name="copper"]').val()) || 0;
        updatedCurrency.sp =
            parseInt($(ev.currentTarget.parentElement.parentElement).find('input[name="silver"]').val()) || 0;
        updatedCurrency.gp =
            parseInt($(ev.currentTarget.parentElement.parentElement).find('input[name="gold"]').val()) || 0;
        updatedCurrency.bank =
            parseInt($(ev.currentTarget.parentElement.parentElement).find('input[name="bank"]').val()) || 0;
        if (game.settings.get('wwn', 'currencyTypes') === 'currencybx') {
            updatedCurrency.ep =
                parseInt($(ev.currentTarget.parentElement.parentElement).find('input[name="electrum"]').val()) || 0;
            updatedCurrency.pp =
                parseInt($(ev.currentTarget.parentElement.parentElement).find('input[name="platinum"]').val()) || 0;
        }
        updatedCurrency = {
    ...
    activateListeners(html) {
        ...
        html.find('button[data-action="adjust-currency"').click(this._adjustCurrency.bind(this));
    }
}
